...
let rec compile (t:term) : cmds = 
  match t with
  | Name n -> [Push (N n)]
  | Unit -> [Push (U)]
  | Int x -> [Push (I x)]

  | Fun (name1,name2,t)-> [Fun (name1, name2, compile t)]
  | Ifgz (t1,t2,t3) ->  compile t1 @ [Ifgz ((compile t2), (compile t3))]
  | LetIn (name,t1,t2) ->[Let (name, compile t1,compile t2)]
  ...
  | Trace t -> (compile t)@[Trace]


let rec conv (cmds) : string = 
  (match cmds with
   | Push x::cmds -> " Push "^const_to_string(x)^conv(cmds)
   | Add ::cmds -> " Add" ^conv(cmds)
   | Sub ::cmds-> " Sub "^conv(cmds)
   | Mul ::cmds-> " Mul "^conv(cmds)
   | Div ::cmds -> " Div "^conv(cmds)
   | Trace ::cmds-> " Trace "^conv(cmds)
   | Call ::cmds-> " Call "^conv(cmds)
   | Ifgz(x,y)::cmds-> " If " ^ " Begin " ^(cmd_list(x)^ " End "^" Else "^" Begin "^cmd_list(y))^" End "^" End "^conv(cmds)
   | Let(x,y,z) ::cmds -> " Begin " ^ " Push " ^ x ^ " " ^ cmd_list(y)^" Let "^cmd_list(z)^" End "^conv(cmds)
   | Fun (x,y,z)::cmds-> " Fun "^x^" "^y^cmd_list(z)^" End "^" Push "^x^" Lookup "^conv(cmds)
   | _ -> ""
  )
and cmd_list (cmds: cmd list) : string  =
  match cmds with
  |[] -> " "
  |h::t -> conv([h])^" "^cmd_list(t)
